...
def load_image():
    ...
    filename = filedialog.askopenfilename(
        initialdir=initdir, title='Select Image')
...
optionScalerl = tk.Scale(secondFrame, label='Resolution Level',
                         from_=1, to=8, orient=tk.HORIZONTAL, resolution=1)
optionScalerl.set(8)
...
optionScaleth = tk.Scale(secondFrame, label='Threshold Value',
                         from_=-50, to=50, orient=tk.HORIZONTAL, resolution=1)
optionScaleth.set(0)
...
def decompose_img():
    ...
    if len(img.shape) == 2:
        ...
        for i in range(3):
            dwt_img[:, :, i] = calc_dwt_gray(
                dwt_img[:, :, i], maxl, minl, th, thFlag)
...
oimg_label3.grid(row=1, column=0)
oimgr = tk.Button(fourthFrame, text='Reconstruct Image',
                  command=reconstruct_img)
